...
def register_care_delete_guards():
    app_config = django_apps.get_app_config("care")

    for model in app_config.get_models():
        pre_delete.connect(
            _prevent_physical_delete,
            sender=model,
            dispatch_uid=f"care.no_physical_delete.{model._meta.label_lower}",
        )
